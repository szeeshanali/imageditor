...
function initUIBanner() {
    ...
    $("#btnUploadBanner").on("click", (e) => {
        ...
        if (bannerId && bannerId.length > 10) {
            if (!confirm("Are you sure you want to update banner changes?")) { return; }
        }

        if (!isValidBanner(bannerName, bannerType)) { return; }
        $loader.removeClass("hidden");
...
function initUIClipArts() {
    ...
    $("#btnUploadClipart").on("click", (e) => {
        ...
        if (!isValidClipArt(clipArtName, category)) { return; }
        $loader.removeClass("hidden");
...
function InitUIEvents() {
    ...
    $("#categoryContainer .btnDelCategory").on("click", function (e) {
        ...
        $.ajax({
            ...
            error: function (res) {
                ...
                if (res.status === 403) { toast(`Category is associated with some of your cliparts, please remove associated cliparts and try again`, 5000); }
                else {
    ...
    $("#btnEditUser").on("click", function (e) {
        ...
        if (confirm("Do you want to save this user changes?")) {
            ...
            let user = {
                is_admin:selectedUser.adm,
                active:selectedUser.act,
                watermark:selectedDesign.wm,
                project_limit:selectedDesign.plmt
            }


            user.is_admin = $("#edit-user-container .is_admin").prop("checked");
            user.active = $("#edit-user-container .is_active").prop("checked");
            user.watermark = $("#edit-user-container .watermark").prop("checked");
            user.project_limit = parseInt($("#edit-user-container .project_lmt").val());
    ...
    $("#btnSaveContent").on("click", function (e) {
        ...
        if (!confirm("Do you want to save Content?")) { return; }
        $loader.removeClass("hidden");
...
function editCustomer(user) {
    selectedUser = user;
    $("#edit-user-container .fname").val(user.unm);
    $("#edit-user-container .lname").val(user.lname);
    $("#edit-user-container .email").val(user.eml);
    $("#edit-user-container .company").val(user.cnm);
    $("#edit-user-container .project_lmt").val(user.plmt);
...
function updateTextControls(e) {
    ...
    if (item.charSpacing) { $("#text-letter-spacing").val(item.charSpacing); }
    if (item.strokeWidth) {
        if ($("#inputStrokeText").is(":checked")) {
    ...
    if (item.lineHeight) { $("#text-line-height").val(item.lineHeight); }
    if (item.stroke) { document.querySelector('#strokecolor') ?.jscolor.fromString(item.stroke); }
...
function getUserSavedProjects(userId, userName) {
    ...
    $.ajax({
        ...
        success: function (res) {
            $loader.addClass("hidden");
            let d = [];
            if (res.data) { showProjectHistory(userId, userName, res.data) }
...
function showDownloadHistory(userId, title, filteredDownloads, type) {
    ...
    let table = `<div>
      <table  id='history-table' class='tx-center table table-striped table mg-b-0 tx-12 tx-bold tx-uppercase' >
          <thead >
              <th class='tx-top-center'>File Name</th>
              <th class='tx-top-center'>Template Name</th>
              <th class='tx-top-center'>Part No.</th>
              <th class='tx-top-center'>Created Date</th>
              <th class='tx-top-center'>Download PDF</th>
              <th class='tx-top-center'>Delete PDF</th>
              </thead>
...
function show_template_usage(template_id, startDt, endDt, templateNm, partNo) {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            show_template_usage_users(res.data)
        }, error: function (request, status, error) {
...
function downloadUserPdf(id, fn, type) {
    ...
    fetch(`/api/pdf/${id}/${type}`, {
        ...
        .then(response => {
            if (!response.ok) {
              toast(`File not found or has been deleted from the server.`);
              return;
            }
