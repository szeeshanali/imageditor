                 ...
                 var con = mysql.createConnection({
                  ...
                  con.connect(function(err) {
                     try {
                      if(err)
                      { 
                        throw 'Error: MySQL Connection Error: ' + err }
                      ...
                      con.query(queryFindUserByEmail,  async function (err, result, fields) {
                        ...
                        if (err) {
                          throw "Error: Database Query Error: " + err;
                         ...
                         if(result == null || result.length == 0){
                          throw 'Error: User email not found in KopyKake DB';  
                          //return done(null, false, { message : 'Incorrect username or password'});   
                         }
                         ...
                         if(!hasher.CheckPassword(password, storedHash))
                         {
                          ...
                          commonService.logger.log(null,1,"login", "Incorrect username or password", 
                          JSON.stringify({user_login:user_login,user_pass:user_pass}),"");  
                          return done(null, false, { message : 'Incorrect username or password'}); 
                        }
                           ...
                           if(!user) {
                            ...
                            // new user model.
                              const newUser = new User(kakePrintUser).save().then((value)=>
                              {
                                ...
                                commonService.logger.log(null,1,"login", "DB_CONN_ERR: KakePrint DB connection Error.", display_name,""); 
                             ...
                             if(user && !user.active) {
                                commonService.logger.log(req.user._id,1,"login", "Account has been blocked by Admin.",display_name,"");
                            ...
                            if(user.password != user_pass ||  user.fname != display_name)
                              {
                                ...
                                User.deleteOne({email : user_email}).then(()=>{
                                  ...
                                  let newUser2 = new User(kakePrintUser).save().then((value)=>
                                  {
                                    ...
                                    commonService.logger.log(req.user._id,1,"login", "Login Success", display_name,"");

                                    return done(null, kakePrintUser); 
                                  }).catch(value=> { 
                                    console.log(value);
                                    commonService.logger.log(req.user._id,1,"login", "Login Failed", display_name ,"");
                              ...
                              commonService.logger.log(user.id,1,"login", "Login Success", display_name,"");
                     ...
                     {
                      commonService.logger.log(req.user._id,5,"login", ex, "","");
                      return done(null, false, { message : "Server Error!"}); 
                     }
                    // if (err) {
